...
def chart(request):
    ...
    if from_date is not None and to_date is not None:  #check if data was supplied by the user
        bitcoin_price, date_from, date_to, wrong_input, search_form = userBtcDataChart(from_date, to_date, wrong_input) #if there is data supplied my the user via the form, proceed to make the api call and retrieve the required data
